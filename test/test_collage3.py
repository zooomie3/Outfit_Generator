...
def find_image_in_closet(item_id, closet_folder="closet"):
    ...
    # Check if the closet folder actually exists first
    if not os.path.exists(closet_folder):
        print(f"⚠️ Error: The folder '{closet_folder}' does not exist here.")
        return None
    ...
    print(f"⚠️ Warning: Could not find any image for {item_id} in {closet_folder}/")
    return None
...
def create_collage(outfit_df, output_filename="final_outfit.png"):
    ...
    for _, row in outfit_df.iterrows():
        ...
        if img:
            ...
            print(f"  -> Added {item_id} to the canvas.")

    # Save the file
    canvas.save(output_filename, format="PNG")
    print(f"✅ Outfit collage successfully saved as {output_filename}!")
